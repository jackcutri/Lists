...
void toStringTest(List* myListPtr){
    ...
    for (int i=0; i<5; i++){
        myListPtr->insertAtEnd(a1[i]);
        printAssertEquals(toStringOutputs[i], myListPtr->toString());
    }
    std::cout << "--done--" <<std::endl;
}
...
//assumes working constructor and insertAtEnd
void findLastTest(List* myListPtr){
...
void runAllListTests(std::string listType){ //this test runs the same no matter if you're testing arraylists or linked lists
